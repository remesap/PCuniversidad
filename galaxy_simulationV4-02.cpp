...
double fe(double & a)
{
  return a*H0*std::sqrt((omega_A + omega_k*(std::pow(a0,2)/std::pow(a,2)) + omega_M*(std::pow(a0,3)/std::pow(a,3)) + omega_R*(std::pow(a0,4)/std::pow(a,4))));
}
...
void forces(std::vector<galaxy> & galaxies, double & a)
{
  ...
  for(int i = 0; i <= N; ++i){
	...
	double xij2 = galaxies[i].r[2] - galaxies[j].r[2];
    	...
    	galaxies[i].f[0] -= (G*galaxies[i].mass)/std::pow(a, 3)*std::sqrt(std::pow( xij0 /( std::pow(xij0, 2) + std::pow(e/a, 2) ), 3));
    	galaxies[i].f[1] -= (G*galaxies[i].mass)/std::pow(a, 3)*std::sqrt(std::pow( xij1 /( std::pow(xij1, 2) + std::pow(e/a, 2) ), 3));
	galaxies[i].f[2] -= (G*galaxies[i].mass)/std::pow(a, 3)*std::sqrt(std::pow( xij2 /( std::pow(xij2, 2) + std::pow(e/a, 2) ), 3));
    }
    }
  }
